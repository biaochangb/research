...
class DataPreprocess:
    ...
    def setEpisodeOrderId(self):
        """设置每集播放记录的序号"""
        self.cursor.execute(
            'SELECT vid,nth FROM youku2014.`epiosde_view_mainland_chain` GROUP BY vid,nth ORDER BY vid,nth')
        episodes = self.cursor.fetchall()
        m = len(episodes)
        j = 0
        for episode in episodes:
            self.cursor.execute(
                'select * FROM youku2014.`epiosde_view_mainland_chain` where vid=%d and nth=%d order by date' % (
                episode[0], episode[1]))
            records = self.cursor.fetchall()
            i = 0
            for record in records:
                self.cursor.execute(
                    'update youku2014.`epiosde_view_mainland_chain` set order_id=%d where id=%d' % (i, record[0]))
                i += 1
            j += 1
            if (j * 100 % m == 0):
                print('%%%d' % (j * 100 / m))
    ...
    def getFirstNDayViewByEpisode(self):
        ...
        for vid in vids:
            ...
            self.cursor.execute(
                'SELECT vid,nth,date, AVG(vv) as vv FROM %s.%s WHERE vid=%d AND order_id<%d GROUP BY nth ORDER BY nth ,DATE' % (
                self.db, self.view_table, vid[0], self.first_n_days))
            records = self.cursor.fetchall()
